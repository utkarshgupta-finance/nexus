...
- **`create_commercial_change_for_configuration`, a legacy ungoverned RPC, remains live at the database level with zero application-layer caller (D-017, Batch 11; E-020, Batch 12).** It performs an unconditional component-closure step identical in shape to the governed `approve_commercial_configuration_version`'s own closure step, with zero awareness of a concurrently in-progress governed draft's seeded snapshot. Confirmed live during the Batches 10-12 evidence reconciliation (2026-09-26) that the function still exists in the schema. Not independently reproduced via a live collision (deliberately, to avoid a real ungoverned mutation against the shared database purely to demonstrate the risk); the risk is fully characterized at the code level. Fix direction: either drop the function entirely (if genuinely dead) or gate it behind the same governed path, once a real decision is made that it should not exist as a bypass.
