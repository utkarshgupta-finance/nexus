...
- **Pre-Batch-19 Reconciliation (2026-09-22): CORRECTED.** The Product Decision Closure fix above still let cancellation delete schedule rows for months not yet reflected in the ledger, i.e. it still stopped future, not-yet-recognized allocation with no Credit Note behind the change. Re-reading the business decision strictly ("Invoice creates entitlement... that entitlement remains valid unless reduced/reversed through a Credit Note... No CN = entitlement remains") applies to past AND future Invoice entitlement, not only to months already reflected in the ledger; stopping future allocation on a mere administrative cancellation is itself an unauthorized reduction. Corrected via migration `20261004000000_fix_cancel_entitlement_source_zero_entitlement_effect.sql`: `cancel_entitlement_source` no longer touches `entitlement_schedule_months` in any way. Cancellation is now a pure administrative/status marker on the `entitlement_sources` row itself (who/when/why recorded, blocks further schedule generation from this specific source), with zero effect on any already-generated schedule, monthly entitlement, or ledger figure, past or future. Live re-verified against ES-000005 (`INV-BATCH17-I017-B`, one ledgered month, eleven not-yet-ledgered months): after cancellation, all 12 schedule rows remained present and unchanged. This is the correct, literal implementation of the decision; only a future Credit-Note-driven mechanism (still a FUTURE MODULE, not built here) may ever reduce Invoice entitlement. The Cancel action's UI confirmation copy was updated to state plainly that it has no effect on any past or future entitlement quantity.
