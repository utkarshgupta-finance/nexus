...
- **DEFECT (Batch 17, I-012)**: resubmitted usage for the same month (qty 999) via the ordinary "Submit Monthly Usage" form immediately afterward. This succeeded with **no error and no warning**, silently superseding the finalized row (`is_current` flipped to `false` on the finalized row, a brand-new `draft` row became current). The finalized row itself remained byte-for-byte immutable (`fn_protect_monthly_usage_lifecycle` correctly blocks direct mutation), but finalization provided **no actual protection** against being superseded by an entirely new row through the normal, unprivileged submission path anyone with `usage.write` can reach. This directly contradicts the journey's own Expected Business Result ("once usage is finalized... it cannot be silently altered") and Expected Technical Invariant. Root cause: `submit_monthly_usage` never checked the status of the row it was about to supersede. Fixed via migration `20261001020000_fix_submit_monthly_usage_finalized_bypass.sql`, adding a check that raises `MONTHLY_USAGE_ALREADY_FINALIZED: usage for <month> has already been finalized and cannot be resubmitted` when the current row for that month is already `final`. Live re-test after the fix (see I-013 below, which reused this same finalized-row scenario before the fix could be re-verified against this exact row) confirms the new guard is real and reachable.
...
- **DEFECT (Batch 17, I-021)**: live-confirmed that when a month's classification flips (here: underuse to overage, after Sep 2026's usage was resubmitted from 25 to 999), `upsert_monthly_entitlement_ledger` correctly updated the new type's entry (`unbilled_ledger_entries`, 949) but **never touched the now-stale entry of the previously-applicable type** (`unearned_ledger_entries`, still showing `quantity = 25`, `status = OPEN`). SQL confirmed both rows coexisting, `OPEN`, for the same month, simultaneously claiming Nexus owed the customer 25 units AND the customer owed Nexus 949 units. This is not the specific "settled month overwritten" scenario the journey's own premise describes (no settlement had occurred against either entry yet), but it is the same class of ledger-integrity failure the journey targets: a stale, contradictory ledger fact left behind by recomputation. Root cause: `upsert_monthly_entitlement_ledger` only ever wrote to a ledger-entry table `if p_<type>_quantity > 0`, never handling the case where a *previously* open entry of that type needs to be zeroed once the recomputed figure for that type drops to zero. Fixed via migration `20261001030000_fix_ledger_stale_unbilled_unearned_entries.sql`: relaxed both tables' `> 0` check constraints to `>= 0`, and the RPC now zeroes an existing OPEN entry of the no-longer-applicable type in the same call (never touching a PARTIALLY_SETTLED/SETTLED entry, and never netting the two types against each other's quantity, per this domain's own explicit no-netting design). Live re-verified after the fix: a further resubmission (999 to 950) correctly left `unbilled = 900` and zeroed the stale `unearned` entry to `0` (both still independently present as OPEN rows, no contradictory nonzero coexistence).
...
- **DEFECT (Batch 17, I-022)**: attempted a second settlement of 1000 against the same entry (900 total outstanding, 500 already settled). This was **accepted with no rejection**, reaching a recorded cumulative total of 1500 against an entry that only ever owed 900, and the entry's `status` incorrectly flipped to `SETTLED` (implying the correct amount was settled, when 600 more than owed was recorded). `record_settlement` never checked a new settlement against the entry's own remaining outstanding quantity before inserting, directly contradicting the journey's own stated invariant ("a settlement_records row cannot reference more quantity than remains outstanding"). Fixed via migration `20261001040000_fix_record_settlement_over_settlement.sql`: computes the already-settled total before inserting and rejects with `SETTLEMENT_EXCEEDS_OUTSTANDING` if the new settlement would push the cumulative total past the entry's own quantity, while preserving the existing idempotent same-reference-retry behavior unchanged. The original 1500-against-900 over-settlement is left in the shared database as inert test evidence (the entry itself, `unbilled_quantity = 900`, was never mutated by the bug, only the sum of its settlement rows is misleading), consistent with this program's established practice of not scrubbing evidence of a found-and-fixed defect.
