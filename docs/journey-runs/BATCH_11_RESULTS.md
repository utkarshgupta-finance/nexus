...
- **Product Decision Closure (2026-09-22): DECIDED.** Business decision: Commercial Configuration will NOT gain an independent deactivate/reactivate lifecycle. `commercial_configurations.is_active` remains permanently unused (schema column present, never written by any code path) by design, not an unbuilt feature awaiting a future batch. If a commercial relationship needs to stop, or the customer should no longer operate under the commercial setup, the customer lifecycle / customer change mechanism (`customers.is_active`) is the correct and only path. This finding required no code change: current behavior (no deactivate path exists) already matches the decided behavior. `docs/NEXUS_JOURNEY_UNIVERSE.md` D-003/D-004/D-015 rewritten to test the permanent absence going forward; D-021 re-scoped to the customer-lifecycle version of its underlying concern (a near-duplicate of the already-decided PD-004/C-030). This historical PRODUCT GAP CONFIRMED finding is preserved above unedited; only this closure note is appended.
...
- **Product Decision Closure (2026-09-21, PD-005 CLOSED): PENDING -> DECIDED -> IMPLEMENTED.** Business decision, reversing this journey's Morning Catch-Up reclassification as "intended current model": do NOT keep global-only authorization; build scoped access now, supporting Global -> Business Unit -> Territory/Geography -> Specific Customer. Implemented in `supabase/migrations/20260930110000_scoped_authorization_foundation.sql`, extending the `user_roles.scope_resource_id` design `docs/AUTHORIZATION_MODEL.md` §5 already anticipated (full detail there, now marked IMPLEMENTED): `customer`, `business_unit`, and `territory` registered as Resource Registry types; `business_unit_resources`/`territory_resources` lookup tables give each already-existing business_unit value and each already-existing `customers.country` value a stable resource identity (backfilled only from real data, kept current by a trigger, never inventing a value); `grant_scoped_user_role` grants a role scoped to exactly one tier; `fn_user_has_customer_scoped_permission` is the actual check (global grant, or a scoped grant matching the customer's own id/business_unit/territory). TypeScript: `hasPermissionForCustomer`/`requirePermissionForCustomer` (`src/platform/permissions/server.ts`), a global holder passes through unchanged with no extra database call, siblings of the existing `hasPermission`/`requirePermission`. Enforcement applied so far to Customer Master (list + detail) and Commercial Configuration (detail) reads via a minimal `AuthGate` extension (`additionalAccessGranted` prop, every other caller unaffected). **Design choice, stated honestly:** the `org_scopes` nested hierarchy table §5 also named was NOT built, since Nexus has no real BU-contains-territory org chart to populate it with; each tier is resolved independently instead (a user's access is the union of every matching grant), still delivering the requested four-tier hierarchy without fabricating a containment tree that does not exist. **Not yet built, explicitly tracked (`docs/TECH_DEBT.md`):** an admin UI screen for granting a scoped role (RPC-only today); enforcement on Customer Onboarding, Customer Change, and Commercial Change reads (applied to 2 of 5 named domains so far). Verified: migration applied (after fixing a real bug caught during application, a `SELECT DISTINCT col, gen_random_uuid()` that does not deduplicate correctly since the random value differs per row; fixed to deduplicate first, then generate exactly one id per distinct value); live-verified read-only (customer resources registered: 19/19 total customers, exact match; business_unit_resources correctly deduplicated to 3 rows across 5 customers with a business_unit set). Unit tests added: `src/platform/permissions/server.test.ts` (global grant short-circuits before any scoped call; scoped-only grant checked via the RPC; denial when neither; every non-active session state denied without a database call). `tsc`/`eslint`/full vitest suite (941 tests) green.
- **Product Decision Closure Phase 3 (2026-09-21):** extended enforcement to all 5 named domains (was 2 of 5): Customer Onboarding (business-unit-only check for pre-customer-existence cases), Customer Change, Commercial Change, plus genuine list/search filtering for Customer Master's own list page and the shared Approvals/My Work/Operational Queue composer, plus the direct-ID onboarding API route. New migration `20260930130000_scoped_authorization_bulk_and_business_unit_checks.sql`. Manually verified live in the browser with a dedicated scoped-only fictional test persona holding no global grant: the Customer Master list showed only the one in-scope customer; direct-ID navigation to an out-of-scope customer was blocked with "Access restricted"; the in-scope customer's own pages loaded normally. Go-live list entries were left deliberately unscoped at this point (never one of PD-005's five named domains); tracked in `docs/TECH_DEBT.md`. Full detail in `docs/journey-runs/OVERNIGHT_PENDING_APPROVALS.md`'s PD-005 entry and `docs/AUTHORIZATION_MODEL.md` §5/§18.
- **Go-live authorization follow-up (2026-09-21):** a bounded check of the go-live tech-debt item above found it was a real, confirmed information-disclosure gap, not harmless debt: `loadApprovalInbox` exposed every go-live entry's customer name, customer key, and status regardless of the viewer's go_live permission, in all three shared surfaces. Live-verified with an existing fictional test account holding a global read grant but zero go_live permission of any kind. The underlying go-live detail route's own independent `AuthGate` correctly blocked direct navigation, so the confirmed leak was list-level disclosure only, never object access or action execution. Fixed using the existing PD-005 mechanism (`getVisibleCustomerIds("go_live", "read")`), no new authorization model; re-verified live after the fix. Full detail in `docs/journey-runs/OVERNIGHT_PENDING_APPROVALS.md`'s PD-005 entry.
...
- **Version chain renders correctly:** all 18 rows of the Version History table rendered in correct sequential order (Version 18 down to Version 1), each with the correct status (Approved/Scheduled or Superseded), category, effective dates, billing currency, and FX snapshot.
...
**Row-count reconciliation (26 vs 18 vs the actual current totals), confirmed exactly rather than guessed:** the underlying request table (every Commercial Change version ever created for this configuration, across every status) currently holds 36 rows total (17 approved, 7 cancelled, 12 rejected); the 26 figure quoted earlier in this same file was simply this same, continuously-growing table's count as of Batch 11's own close, before the additional live testing performed in Batch 12 and this reconciliation pass added more rows. That request table is a governed workflow ledger: it records every attempt, including ones a reviewer rejected or an analyst cancelled, and was never meant to be the customer-facing history. The Version History table the page renders is built from a separate, second table (the applied Commercial Change history, populated only when a request actually reaches final approval, plus the one genesis row created together with the configuration itself): confirmed by reading the exact function that builds the page's row list, which numbers its rows purely by ordering that second table's rows chronologically, one row per entry, no filtering or pagination beyond that. That second table currently holds exactly 18 rows for this configuration, matching the page's own "Version 18" label precisely. The 18 shown on the page is not a truncated subset of a larger set that should have appeared; it is 100% of what the table is designed to render, and a cancelled or rejected request correctly never reaches that table at all, by the same design already confirmed correct elsewhere in this program (a rejected or cancelled request never becomes real, applied commercial history). No omission. D-020's own objective (does the page render its full, real chain correctly) is fully and exactly satisfied.

D-020 reclassifies its evidence from structural/DB-count only to `MANUAL UX VERIFIED` (browser) plus `DATABASE VERIFIED` (the underlying row count, now exactly reconciled rather than approximated). **Final status unchanged: PASS.**
