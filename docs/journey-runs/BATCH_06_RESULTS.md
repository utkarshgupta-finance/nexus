...
### ADDENDUM 2026-09-24 (P-002 through P-011, P-015 CLOSED): genuine live evidence for the remaining Reference Master journeys

Continuing directly from P-001's closure, on the same working admin tab.

- **P-002 (Business Unit add):** Typed a fresh disposable label into the Business Unit tab's Add form, clicked Add. Confirmed via SQL: `reference_options` row `code='p_002_genuine_add_bu'`, `list_key='business_unit'`, `is_active=true`, rendered immediately in the live list. PASS.
- **P-003 (Industry/Category add):** Same pattern on the Industry/Category tab. Confirmed via SQL: `code='p_003_genuine_add_industry'`, `list_key='industry'`, `is_active=true`. PASS.
- **P-004 (Tax Identifier Type add):** Same pattern on the Tax Identifier Type tab. Confirmed via SQL: `code='p_004_genuine_add_tax_id'`, `list_key='tax_identifier_type'`, `is_active=true`. PASS.
- **P-005 (deactivate an unreferenced Level 1 value) / P-007 (reactivate):** Reused the disposable P-001 Segment value (genuinely unreferenced by any customer). Clicked "Deactivate", clicked the row's own "Confirm" step (this control uses an inline confirm, not an instant toggle), confirmed via SQL `is_active` flipped `true` -> `false`. Clicked "Activate", confirmed via SQL `is_active` flipped back to `true`. Both PASS.
- **P-006 (deactivate a Segment value referenced by an approved customer):** Used the existing approved customer `aurora-consumer-labs` (`segment='mid_market'`, the only active customer on this value, chosen to minimize blast radius). Baseline screenshot: customer detail page renders "Segment: Mid Market". Deactivated "Mid Market" in Reference Master (confirmed via SQL). Reloaded the customer page: "Segment: Mid Market" rendered identically, unconditionally, no warning/error badge. Reactivated "Mid Market" (confirmed via SQL `is_active` restored to `true`). PASS, this program's core data-integrity guarantee held.
- **P-008 (Currency add with inrConversionRate):** Added currency code "AUD" (label typed with an unintended self-inflicted duplicate prefix, a cosmetic input mistake, not a defect) via the Currency tab's two-step flow (add code+name, then set the rate in the newly-appeared row). Confirmed via SQL: `code='AUD'`, `is_active=true`, `inr_conversion_rate` initially null, then set to `54.25` after the second step, both steps genuinely reflected in the DB. PASS.
- **P-009 (Currency rate update, non-retroactivity):** Found an existing, currently-effective (`effective_to is null`) commercial component (`f60b43a7-...`, customer `batch8-approval-core-co`) with `transaction_currency='USD'`, `fx_snapshot_rate=83.25`. Recorded USD's live reference rate baseline (91). Updated USD's rate to 92.75 via the Currency tab's inline rate field (confirmed via SQL). Re-queried the existing component: `fx_snapshot_rate` still exactly `83.25`, completely untouched. Reverted USD's rate back to `91` (confirmed via SQL). PASS, the non-retroactivity guarantee held.
- **P-010 (deactivate a Currency referenced by an active commercial component):** **Process mistake caught and corrected inline:** the first "Deactivate" click on what was intended to be USD's row actually landed on INR's row instead (list order had shifted after the P-009 edits); confirmed via the dev server's own log (`setOptionActiveAction("currency", "INR", false)`), not assumed. Immediately reactivated INR via the same governed UI action, confirmed via SQL `is_active` restored to `true` before proceeding, and confirmed via a fresh `get_page_text` read that no other state had shifted. Retried correctly against USD's own row this time (verified via a fresh, unambiguous `get_page_text` row-order read before clicking): dev server log confirmed `setOptionActiveAction("currency", "USD", false)`. Opened the referencing customer's full Commercial Configuration page: "Billing Currency: USD", "FX Snapshot: 1 USD = INR 83.25" both rendered exactly, no warning/error badge, matching the version history's own already-recorded historical rate. Reactivated USD (confirmed via SQL). PASS, both the mistake-recovery and the canonical assertion itself are now on record.
- **P-011 (Invoice Frequency add with cadenceMonths):** Added "P-011 Genuine Frequency" with cadenceMonths=9 via the single-step Invoice Frequency add form. Confirmed via SQL: `code='p_011_genuine_frequency'`, `is_active=true`, `cadence_months=9`. PASS.
- **P-015 (deactivate a Level 3 value referenced by an existing commercial component, Revenue Recognition Method):** Found an existing, currently-effective non-recurring commercial component (customer `test-customer-1`) whose `pricing_rule_parameters.revenueRecognition.method = "milestone_based"`, with a real two-milestone breakdown (50%/50%, INR 5,000,000 each). Deactivated "Milestone Based" in the Revenue Recognition Methods tab under System Rules (confirmed via dev server log `setOptionActiveAction("revenue_recognition_method", "milestone_based", false)`). Opened the customer's Commercials page: the "Non-Recurring Commercials" table rendered the full milestone breakdown ("Milestone 1, 50%, INR 5,000,000", "Milestone 2, 50%, INR 5,000,000") exactly as before, no warning/error badge. Reactivated "Milestone Based" (confirmed via SQL `is_active` restored to `true`). PASS, the same historical-preservation guarantee holds for Level 3 values.
- **Defect found?:** No new product defects across any of P-002 through P-015. (The two issues found this addendum, the AUD label self-duplication and the INR/USD row mixup, were both my own process mistakes, caught via server-log/SQL corroboration and corrected before they affected any evidence, per this program's evidence-discipline standard.)
- **Journey Discovery observation:** ALREADY COVERED for all of P-002 through P-011 and P-015.
- **Permanent ledger updated:** Yes (this entry, plus the Batch Report/Summary Metrics tables below).

### END ADDENDUM (P-002 through P-011, P-015: all CLOSED, PASS)

---

### ADDENDUM 2026-09-24 (O-018, O-024, O-025 CLOSED): genuine live evidence, O-021 parked on a persona-login blocker

**O-018 (last remaining active team member removed while a request waits, warn but allow).** Fixture: `wf_test_finance` genuinely had exactly one active member (`Nexus Test Finance Approver`) and real, currently-pending `customer_change`/`go_live` requests at nodes responsible to that team (confirmed via direct query before touching anything). As the real super-admin (holds `team.write`), clicked "Remove WF-TEST Finance" on that persona's row on `/settings/user-access`. Dev server log confirmed the real server-side sequence: `checkTeamRemovalImpactAction(...)` fired first, immediately followed by `removeUserFromTeamAction(...)`, exactly matching the `handleRemoveTeam` client code path (`src/platform/user-access/ui/user-access-page.tsx`), which only proceeds straight to removal without interruption when there is no pending-approval risk, or after a native `window.confirm()` the user accepts. Disclosed limitation: this session's browser tooling cannot make the native `window.confirm()` dialog's own text visible to `read_page`/screenshot (it is rendered by the browser engine, outside the DOM), so the exact warning count could not be visually captured; the server-side call sequence is the strongest evidence available for that specific sub-step. The Recovery/Resilience Variant was fully, visually confirmed: `/operations/queue` showed "38 items have no eligible approver" with many real rows now reading "WF-TEST Finance / No eligible approver" immediately after the removal. Restored the membership via the same real UI/RPC path (`assign_user_to_team`, called directly for baseline-restoration only, not as evidence for the assertion under test) and re-checked `/operations/queue`: banner dropped to "19 items," and the previously-flagged `WF-TEST Finance` rows no longer showed the badge. Confirmed via SQL: `wf_test_finance` active member count is back to exactly 1. PASS.

**O-024 (new team is immediately assignable).** Created a brand-new fictional team, `o024_fresh_team` / "O-024 Fresh Team", through the real "Add Team" form on `/settings/teams` (confirmed via dev server log `createTeamAction("o024_fresh_team", "O-024 Fresh Team", null)`). Confirmed via SQL immediately after: `is_active = true` with no separate activation step. In the same browser session, immediately navigated to `/settings/user-access` and assigned this brand-new team to `Nexus Test Restricted User` via the real "Assign a team" control (confirmed via dev server log `assignUserToTeamAction(...)` with the new team's id), and confirmed the rendered page showed "O-024 Fresh Team (Primary)" against that user. No activation step was needed or possible; none exists in the UI. Restored baseline by removing this membership afterward (confirmed via dev log `removeUserFromTeamAction(...)`); the fictional team itself was left in place, matching the existing convention of every other permanent `WF-TEST`-style fixture team already in this environment. PASS.

**O-025 (two admins simultaneously assigning the same user to the same team).** A literal simultaneous double-click cannot be produced through browser automation (no tool can guarantee two clicks land in the same instant), so, consistent with the concurrency-testing precedent already established and accepted earlier in this exact batch (the original O-025 pass), this was tested by firing two genuinely concurrent calls to the real `assign_user_to_team` RPC, the exact same function the UI's "Add" button invokes, via `Promise.all` from a one-off script against a fresh, real user/team pair (`nexus-test-maker@example.test` / the same `o024_fresh_team` fixture, both already confirmed clean beforehand). Both concurrent calls returned the identical row id, and a direct SQL check afterward confirmed exactly one active (`revoked_at IS NULL`) `user_teams` row for that pair, no duplicate. Restored baseline via the real `remove_user_from_team` RPC; confirmed `nexus-test-maker@example.test` back to "no team assigned." The one-off script was deleted after use, per this program's no-stray-artifacts rule. PASS.

**O-021 (team membership assignment requires `team.write`, separate from `user_access.write`).** Parked. See the "O-021: WAITING FOR CANONICAL USER ACCESS ADMIN LOGIN" note above and the consolidated Human Action List at the end of this closure run for exactly what remains and why. The already-captured evidence (a `team.write`-only persona is denied at the `/settings/user-access` page level for lacking `user_access.read`, a stronger enforcement than the canonical spec's field-level denial) is preserved and not weakened; the converse assertion (`user_access_admin` without `team_admin` specifically denied on the team-assignment action) still needs a genuine login as the new `nexus-test-user-access-admin@example.test` canonical persona, which this session's browser tooling could not complete (native-input state on the login form would not reliably register through this tool in this session, confirmed via `document.querySelector(...).value` read-backs showing empty fields immediately after typing, with zero corresponding network requests).

**Journey Discovery check:** No new gaps found in O-018, O-024, or O-025 beyond what is already on record (O-018's own product gap was already closed in an earlier pass, see the entry above). The disclosed native-`window.confirm()`-visibility limitation and the login-form-input-registration limitation are both tooling constraints of this session's environment, not product findings, and do not themselves warrant a journey or defect entry.

**Permanent ledger updated:** Yes (this entry, plus the Batch Report/Summary Metrics tables below, already updated to reflect these three closures and O-021's parked status).

### END ADDENDUM (O-018, O-024, O-025: CLOSED, PASS; O-021: WAITING FOR CANONICAL USER ACCESS ADMIN LOGIN)
...
| P-001 through P-011, P-015 | CLOSED 2026-09-24: genuine live add/deactivate/reactivate on fresh tabs, confirmed via SQL and dev-server-log RPC evidence for every value (see addenda above) | PASS | ALREADY COVERED |
...
**ADDENDUM 2026-09-24 (historical note, superseded by the closures above):** an earlier pass this same day found every remaining residual blocked by a session-wide click-delivery degradation. That root cause was subsequently resolved (fresh tabs register clicks reliably; see `BATCH_07_RESULTS.md`'s addendum for the decisive test), and P-001 through P-011, P-015, O-018, O-024, and O-025 were then closed with genuine live evidence as recorded above.
