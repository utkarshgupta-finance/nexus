...
### METHODOLOGY CLARIFICATION (user-directed, applies retroactively to K-017 and permanently going forward)

**Synthetic DOM events (dispatched `pointerdown`/`pointermove`/`pointerup`, programmatic `.click()`, JS-composed drag sequences) are diagnostic and fixture-preparation tools. They do not, by themselves, constitute Manual UX evidence for the specific user-visible interaction the canonical assertion is about.** They remain legitimate for: diagnosing event wiring, understanding why a genuine interaction fails, and preparing fixture state (building a graph, entering text) when the canonical assertion under test is something else entirely.

Applied to K-017 above: the canonical assertion is "a user holding only `workflow_admin` can complete the full create-to-activate lifecycle unassisted, hitting no permission rejection" — a permission/role-authorization boundary, not a graph-authoring-mechanics assertion (confirmed by re-reading `docs/NEXUS_JOURNEY_UNIVERSE.md` K-017: Regular Path is "User performs every step of the lifecycle without ever hitting a permission rejection," with no mention of proving the drag-to-connect interaction itself). The PASS verdict stands on that basis: the create/save/publish/activate actions were genuinely exercised through this persona's own real, independently-verified browser session, and the correct actor identity was independently confirmed server-side (not inferred from the click mechanism used to trigger it). The Start->End edge, however, was built using the synthesized pointer-event drag recipe as **fixture preparation only** — it demonstrates that a graph could be constructed to satisfy the "save a valid draft" precondition, not that this persona (or any persona) can genuinely drag-to-connect two nodes via real mouse interaction. That specific mechanic (genuine mouse-drag edge creation) was already independently covered with real interaction elsewhere in this file's history (K-022's rename-survives-topology-change assertion in this same continuation pass also used the synthetic recipe for its supporting edges, which is a similar fixture-preparation use, not re-litigated here per the user's explicit instruction not to reopen already-covered edge-authoring mechanics unnecessarily). No further residual is opened for K-017 on this basis, since its own canonical text does not require proving edge-drawing UX under the workflow_admin persona specifically.
