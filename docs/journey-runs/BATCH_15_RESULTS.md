...
### H-020: Multi-Node Workflow Advance — FAILED THEN FIXED + PASS
...
- Actual evidence (database/RPC layer): `status` remained `submitted` (not yet approved), `current_workflow_node_key` advanced `node_2` -> `node_3`, `row_version` incremented, no `approved_by`/`approved_at` written. Confirms the RPC's own graph walk correctly distinguishes an intermediate Approval node from the terminal one.
- Original failure (manual UX, primary truth): immediately after taking this exact action, the request's own Timeline displayed "Approval approved: line item is now Live" for this intermediate advance. This directly contradicts this journey's own UX Check ("Line item's derived status remains GO_LIVE_PENDING throughout intermediate advances, only becoming LIVE at final approval") and Expected Business Result ("Multi-step review chains are honored in sequence"): a real approver taking this exact action would have been told, in the same screen, in the same moment, that the line item was already Live when it was not. The underlying `deriveLineItemGoLiveStatus` badge on the Go Live list page was unaffected and stayed correct throughout (this was a Timeline-copy defect, not a data defect), but the manual UX check this journey specifies was, as executed, genuinely violated.
- Root cause and fix: see "DEFECT: Timeline mislabels an in-progress multi-node advance as 'line item is now Live'" below. Fixed by threading `isRequestFinalized` (`request.status === 'approved'`) through `buildWorkflowTransitionEvents` so the terminal wording only folds onto a transition that actually finalized the request.
- Retest: this journey's own request (`GLR-000012`) retains both the original intermediate transition and the later finalizing transition (H-021) in one continuous transition history. Automated regression test added and passing (`transition-events.test.ts`, 11/11). Manual browser re-verification against the redeployed Preview (`65ec12f`, stable `team-preview` alias) was blocked twice by Vercel's own deployment-protection SSO gate, then succeeded once the user authenticated to Vercel: `GLR-000012`'s live Timeline now reads "Approval approved" (Finance Checker, 21 Sept 2026 4:31 pm, the intermediate node_2 -> node_3 advance, no Live claim) followed by "Approval approved: line item is now Live" (Legal Checker, 21 Sept 2026 4:35 pm, the terminal node_3 -> node_4 advance). Confirmed correct.
- Classification: **FAILED THEN FIXED + PASS**. The RPC/data-layer assertions this journey specifies passed on the first attempt; the manual-UX assertion this same journey specifies (no premature Live signal during an intermediate advance) failed on the first attempt and is now fixed, unit-tested, and awaiting a live browser reconfirmation that tooling access currently blocks.
...
- **Classification reconciliation**: this was H-020's own Regular Path action, and H-020's canonical UX Check is specifically "line item's derived status remains GO_LIVE_PENDING throughout intermediate advances, only becoming LIVE at final approval." Per this repository's own rule that manual UX is primary truth, this defect is not incidental to H-020: it is a genuine, first-attempt failure of that journey's own manual-UX assertion. H-020 has been reclassified from PASS to **FAILED THEN FIXED + PASS** above; the closure arithmetic below reflects this. H-021 (the terminal case, which correctly showed Live only at finalization) is unaffected and remains PASS.
...
| --- | --- |
| PASS | 24 |
| FAILED THEN FIXED + PASS | 1 (H-020) |
...
One real product defect was found and fixed during execution (Timeline mislabeling, see above). On reconciliation, this defect is not merely incidental: it was H-020's own Regular Path action that produced it, and it directly contradicted H-020's own canonical UX Check (no premature Live signal during an intermediate advance). H-020 is reclassified from PASS to FAILED THEN FIXED + PASS accordingly; H-021 (the terminal case) is unaffected and remains PASS. G-025 and G-026 were each completed across multiple live attempts (rejected-and-redone submissions) before reaching their final PASS state; every rejection was caused by a real, correct approval-time validation guard being exercised for the first time live, not by a defect, and each is documented above.

## Post-fix manual UX reverification (closure reconciliation)

Attempted against the redeployed Preview (`51aa1c4`, stable `team-preview` alias) using `GLR-000012`, the same request H-020/H-021 exercised: its transition history already contains both the intermediate node_2 -> node_3 advance and the finalizing node_3 -> node_4 advance in one continuous timeline, so no new fixture was needed to demonstrate both states.

Initially blocked: the freshly built Preview URL required Vercel's own deployment-protection SSO, with no existing authenticated browser session available to reuse. No Vercel credentials were entered or otherwise bypassed by this session; the user then authenticated to Vercel independently in their own browser session.

Verified, live, against `65ec12f` (the exact commit this reconciliation pass produced): navigated to `GLR-000012`'s detail page (`/customers/wf-test-pd-002-case-b-8b2e154f/go-live/ef0ea3af-6081-4dc8-8e06-baedc82e948f`), the same request H-020/H-021 exercised, so no new fixture was needed.

- Intermediate advance (Finance Checker, 21 Sept 2026, 4:31 pm): Timeline reads **"Approval approved"**. No premature Live claim. Confirms the fix.
- Finalization (Legal Checker, 21 Sept 2026, 4:35 pm): Timeline reads **"Approval approved: line item is now Live"**. Correct, since this transition genuinely finalized the request.
