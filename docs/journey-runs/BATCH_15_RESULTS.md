...
Blocked: the freshly built Preview URL requires Vercel's own deployment-protection SSO, and no existing authenticated browser session survived from earlier in this run to reuse. No Vercel credentials are available to this session, and per this repository's access-control rules, none were entered or otherwise bypassed. This is a tooling/session limitation, not a product issue. What remains verified: the automated regression test in `transition-events.test.ts` reproduces this exact scenario (intermediate transition, `isRequestFinalized` false, terminal text withheld) and passes; `tsc`, `eslint`, `vitest`, and the production build are all clean against the deployed commit.
