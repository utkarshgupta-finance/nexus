...
- Priority: P1
- Automation Feasibility: FULL
- Test Data: a real Commercial Change version submitted with a `linear` component carrying a rate of `-50`, approved via direct RPC call (bypassing the TS `isCommercialRateDraftComplete`/`isPositive` validation gate that would normally run inside the real service layer before ever reaching the RPC).
- Actual Result: **the negative rate was accepted and persisted, with zero rejection anywhere in the RPC or database layer.** Confirmed via code reading beforehand and now empirically live-verified: `pricing_rule_parameters` is untyped `jsonb` with only key-presence shape checks at the DB level (no numeric CHECK constraint of any kind), and the only real enforcement of non-negative values exists in the TypeScript service layer (`isPositive`, `commercial-rate.ts:684-686`), invoked once, specifically at version-approval time (`commercial-version.service.ts:120`), never at draft-save time (a negative value can be freely saved into a draft, only flagged `severity: "incomplete"`, not `"invalid"`) and never as a database-level guarantee.
- Classification: this is a real, confirmed gap, but the responsible migration's own comment (`20260908210000...sql:534-537`) explicitly and knowingly defers "deeper pricing-parameter semantics (tier ordering, non-negative rates)" to "the domain service / Pricing Kernel," meaning this is a documented, deliberate architectural deferral, not a silent oversight. It is nonetheless a real trust-boundary gap: any direct RPC caller (a compromised service-role credential, a future integration, or a bug in a not-yet-written API client) can persist a negative commercial rate with zero backstop, directly relevant to `CLAUDE.md`'s stated principle that finance controls should be "database-enforced defaults, not conventions."
- Final Status: PRODUCT GAP CONFIRMED
- Notes: recorded in the final report; not fixed this run since it is explicitly out of this mission's "bounded defect" scope (adding real numeric/business-rule validation inside `pricing_rule_parameters` jsonb would mean either a broad set of new CHECK constraints per pricing kind or a dedicated "Pricing Kernel" validation layer, both meaningfully larger than a bounded fix, and the deferral is already a documented, deliberate architectural choice rather than an accidental gap).
