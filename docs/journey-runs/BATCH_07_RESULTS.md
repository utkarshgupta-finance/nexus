...
**Genuine defect discovered, disclosed, NOT yet fixed (deliberately, see below):** while investigating CO-000103's final state, its `city` field was found `null` in both frozen submitted revisions, despite having been set (`"57702"`, i.e. Anekal, Karnataka) and independently confirmed via a direct DOM value read immediately before the "Save Draft" click that should have persisted it. The dev server log shows two `saveOnboardingDraftAction` calls fired back-to-back for the same request and stage, with expected row_versions 7 and 8 respectively: the first carried 16 data items (including city), the second carried only 15 (city missing). Because each call's expected row_version matched the version the server was actually at when it arrived, optimistic locking did not reject either one; the second, stale-looking call silently overwrote the first's correct data. This is a genuine data-integrity defect (a user can see a field hold in the UI and have it silently vanish from what is actually persisted), not a tooling artifact: it was found through the same "isolated single-variable retest" discipline this session's own browser-automation-quirks memory prescribes, and it survived that retest. Root cause is not yet conclusively established (a plausible but unconfirmed hypothesis: selecting a City value may re-fire a State value-changed event, and `handleValueChanged` unconditionally clears City whenever State changes). Rather than force an unverified fix into this already-long session, this was flagged via `spawn_task` (`task_f26a8445`) for dedicated, properly-scoped investigation, consistent with this ledger's standing practice of not landing a fix without first genuinely confirming its root cause. Did not affect the validity of A-001 through A-004's closure above: Customer Details was independently confirmed structurally complete (not flagged as incomplete) at both Submit attempts, since Reference Master field-completeness validation does not require `city` specifically.
