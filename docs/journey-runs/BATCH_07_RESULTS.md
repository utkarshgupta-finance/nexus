...
**Genuine defect discovered, disclosed, and now FIXED:** while investigating CO-000103's final state, one non-identifying, non-required draft field was found unexpectedly absent from what was actually persisted, despite having been set and independently confirmed present immediately before the save that should have persisted it. Rather than force an unverified fix into that already-long session, it was handed off via `spawn_task` (`task_f26a8445`) for dedicated, properly-scoped investigation. That investigation found two distinct, real causes in the Customer Details City field's own handling, applied a bounded fix to each, and added regression coverage (a failing-without-the-fix repro, a passing-with-the-fix check, and a control test ruling out the same failure mode for a neighboring field whose data path differs). This session independently re-verified the fix live: created a fresh draft, set City, saved, navigated to another stage and back, reloaded the page cold, and saved again, confirming via direct DB reads after every step that City survived (previously, this exact sequence silently wiped it). `tsc`/`vitest` clean (1023/1023). Fix commit: `828111b`.
