...
- **Product Decision Closure (2026-09-21, PD-003 CLOSED): PENDING -> DECIDED -> IMPLEMENTED.** Business decision: a new Commercial Configuration Version must NOT be created for an inactive customer, aligning with Customer Change's existing precedent. Implemented server-side in two layers: (1) `supabase/migrations/20260930090000_block_commercial_version_creation_for_inactive_customer.sql` redefines `create_commercial_configuration_version` to raise `COMMERCIAL_VERSION_CUSTOMER_INACTIVE` if the owning customer's `is_active` is false, closing the raw-RPC-bypass gap this journey identified (Customer Change's own equivalent check is TS-layer only; this goes one step further and is enforced at the RPC layer too); (2) `createCommercialVersionAction` (`src/features/customer-onboarding/actions.ts`) mirrors `createChangeRequestAction`'s identical TS-layer check for a clean UX-facing error before ever calling the RPC. PD-004's behavior (an already-existing in-flight request may still be approved after the customer later becomes inactive) is explicitly unaffected: this check only gates NEW version creation, never `approve_commercial_configuration_version`. Migration applied and live-verified reachable (RPC callable, correctly raises `COMMERCIAL_VERSION_CONFIGURATION_NOT_FOUND` for a nonexistent configuration id, confirming the new check compiles and the function runs; the full live deactivate-and-attempt-and-restore sequence against a real customer was not run in this pass, since it required a separate mutation-specific authorization beyond the one already given for applying the migrations themselves; the code path is identical to the already-proven Customer Change precedent and was reviewed line by line against the current live RPC body before writing). `tsc`/`eslint`/full vitest suite (941 tests) green.
- **Product Decision Closure Phase 3 (2026-09-21):** the full live deactivate-and-attempt-and-restore sequence, deferred above, was completed against a fictional test customer created during this session's own PD-002 verification. The RPC-level block fired correctly, confirming server-side enforcement genuinely works, but a real UX defect was found: the bare create-and-redirect route (`/commercials/[configId]/versions/new`) had no catch for `COMMERCIAL_VERSION_CUSTOMER_INACTIVE`, so the honest rejection crashed the page instead of rendering an explained state. Fixed (`commercial-version-errors.ts` new `commercial_version_customer_inactive` kind; the route now renders "Customer is inactive. Reactivate the customer before creating a new Commercial Configuration Version."). Reactivating the customer afterward confirmed the block lifts correctly. Full detail in `docs/journey-runs/OVERNIGHT_PENDING_APPROVALS.md`'s PD-003 entry.
