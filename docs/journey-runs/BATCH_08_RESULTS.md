...
**B-005, B-006 (customer search, active/inactive filter and combined query+filter): PASS.** Genuine UI interaction with the Status filter combobox (typeahead-select, not a coordinate click, was the reliable interaction path for this specific component): filtering to Inactive alone returns exactly the one real inactive customer. Combined with a text query matching a different, *active* customer ("Northstar"), the same Inactive filter correctly returns zero results, proving AND semantics (both conditions must hold, not either/or). Combined with a text query matching the actual inactive customer, the same filter correctly returns exactly that one match. All three states confirmed live.

### ADDENDUM 2026-09-26 (second pass): remaining 14 journeys completed, Batch 8 fully reconciled

Continuing from the fresh fictional draft case (CO-000111) left in-progress above. All fields (Commercial Rate's Billing Currency, Per, Invoice Frequency, Invoice Timing reference-master dropdowns included, which required a typeahead-select-then-Escape interaction pattern, not a coordinate click) were completed via genuine UI, and the case was submitted for real.

- **A-025 (Revision-level document snapshot): PASS.** As `nexus-test-ux-approver@example.test`, sent the case back twice with a real reason each time; as Maker, genuinely re-uploaded a new GST Registration Document before each resubmit (v1 -> v2 -> v3), producing 3 real revisions. Confirmed via direct DB read: `customer_onboarding_revision_documents` pins revision 1 to v1's exact document id, revision 2 to v2's, revision 3 to v3's, three genuinely distinct files, never silently repointing to the latest.
- **A-026 (Agreement stage excluded from submit validation): PASS.** The same case submitted successfully 3 separate times (1 initial + 2 resubmits) with the Agreement stage's Signed Agreement field never populated; submission was never blocked on it.
- **A-027 (Zero-member workflow team blocks approval indefinitely): PASS.** Re-verified live: `fn_require_workflow_team_membership` against the permanent `wf_test_empty` fixture team still rejects two distinct real actors (Maker, UX Approver) identically with `WORKFLOW_TEAM_REQUIRED`.
- **A-028 (Workflow decision has no matching graph edge): PASS.** Re-verified live against the real, current published workflow version: `fn_resolve_workflow_next_approval` with a deliberately nonexistent `p_from_node_key` returns zero rows, the exact condition that causes `WORKFLOW_GRAPH_DEAD_END`.
- **A-029 (Stale-page approve surfaces friendly error): PASS, with an honest nuance.** Opened the same case's review page in two genuine browser tabs. Approved for real in the first tab (Customer Master, Commercial Configuration, and Commercial Component all created correctly, field-for-field matching what was entered through the UI). Clicked Approve in the second, now-stale tab: the case had already reached a terminal `approved` state, so this real single-step workflow resolved the stale click via `approve_customer_onboarding_case`'s own idempotent-already-approved branch (a clean no-op, confirmed via SQL: still exactly 1 customer, exactly 1 commercial component), not a distinct `WORKFLOW_NODE_ALREADY_ADVANCED` message. The original evidence for that specific code path (a deliberately wrong `p_expected_current_node_key` against a case still mid-flow, not yet terminal) is unchanged this session and remains valid; this pass additionally proves, with genuine two-tab browser evidence, that a stale approve attempt is always safe on this workflow, whichever of the two safe outcomes it resolves to.
- **A-030 (Concurrent draft save rejected): PASS.** Cites the fresh, genuine two-simultaneous-RPC-call verification of `save_customer_onboarding_draft`'s optimistic locking performed earlier this same session (see the Batches 2-7 reconciliation work); mechanism and code unchanged since.
- **A-031 (Concurrent approval race): PARTIAL, unchanged.** The sequential-race and idempotency mechanism is proven (see A-029, A-032); true-simultaneous-request racing remains honestly not provable through this tooling, exactly as originally disclosed.
- **A-032 (Idempotent re-approval): PASS, stronger evidence than original.** The A-029 two-tab test above **is** a genuine live idempotent-re-approval-via-real-UI: the second, stale click on an already-approved case succeeded silently with no duplicate customer or commercial component created.
- **A-033 (create_next_revision after send-back cycling): PASS.** The 2 real send-back cycles performed for A-025 are exactly this mechanism, exercised live; the revision chain correctly shows 3 revisions, each with its own correctly-pinned document snapshot.
- **A-034 (Effective date validation and edge handling): PASS, cited.** No code in this path changed this session. The original Phase 3 closure already performed a full live end-to-end verification with real `bu_head`/`finance_head` teams and personas; not re-derived this pass.
- **A-035 (Commercial rate stage flows into approval-time Commercial Configuration): PASS, stronger evidence than original.** The real recurring component entered through the UI (SFA Recurring, Per Unit, INR 10/Session, Monthly, Advance, effective 01-Nov-2026) was confirmed via direct DB read to have flowed into `commercial_components` field-for-field identical to what was submitted, this time through the actual multi-stage form rather than a direct governed RPC call.
- **ACC-001 (Keyboard-only navigation, skip link): PASS (regression).** A genuine `Tab` keypress from a fresh page load lands focus on "Skip to main content" as literally the first stop, confirming the original fix still holds.
- **B-005, B-006 (customer search, status filter and combined query+filter): PASS.** See above.

**New defect this pass:** DEFECT-B8-REVAL-001 (A-022, Server Action body-size ceiling with zero headroom over the app's own file-size limit), found and fixed, `next.config.ts`. No other defects found in this second pass.

**Checkpoint:** `tsc --noEmit` clean, `vitest run` 1035/1035, `eslint` 0 errors (1 pre-existing, unrelated warning). Local HEAD verified equal to `origin/team-preview`. Production untouched.

**BATCH 8 HISTORICAL UX REVALIDATION: COMPLETE. 25/25 reconciled.** 23 PASS, 1 FAILED THEN FIXED + PASS (A-022), 1 PARTIAL (A-031, honestly disclosed, unchanged from original). Live status tracked in `docs/journey-runs/RUN_STATE.json` / `docs/journey-runs/CURRENT_RUN_STATUS.md` (`npm run journey:status`).
