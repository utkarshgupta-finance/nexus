...
| `FilterSelect` | **IMPLEMENTED** (`src/components/product/filter-select.tsx`, Platform Operating Expansion, Phase F). The one shared "filter by X, or All" select: closes a real defect where an internal `"__all__"`-style sentinel value leaked as the visible trigger label on first paint (Base UI's `SelectValue` resolves its text from a lazily-populated items registry, empty before the dropdown has ever been opened, so it fell through to rendering the raw sentinel string). Computes its label directly via a pure, independently-tested `resolveFilterLabel` function instead of relying on that registry. Reused by Customers today; the one place any future Approvals/My Work/Operations Queue/Settings filter with an "All" option should be built from, not a per-screen reimplementation. |
