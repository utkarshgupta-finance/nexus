-- Nexus: Platform Operating Expansion, Phase H follow-up.
--
-- approve_customer_onboarding_case (20260916020000_customer_master_governed_fields.sql)
-- added every new governed column to the Customer Master insert except
-- brand_name, a real customers column since
-- 20260913060000_customer_change_request_foundation.sql that had never
-- actually been populated at customer creation by any code path. Same
-- exact 10-parameter signature as the version just created (no overload
-- risk): only the insert's column/value list changes, adding brand_name
-- sourced from the same p_customer_fields jsonb payload every other new
-- field already uses.
--
