...
async function ensureAuthUser(supabase: ReturnType<typeof createClient>, email: string) {
...
async function main() {
  ...
  const unprovisioned = await ensureAuthUser(supabase, "wf-test.unprovisioned@example.test")
  ...
  const inactive = await ensureAuthUser(supabase, "wf-test.inactive@example.test")
