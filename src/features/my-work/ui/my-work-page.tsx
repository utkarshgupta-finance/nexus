...
function MyWorkPage({ items }: { items: MyWorkItem[] }) {
  ...
  return (
    <div className="flex flex-1 flex-col">
      <PageHeader title="My Work" description="Requests sent back to you, and requests waiting on your approval." />
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-4 sm:px-6 sm:py-6">
        {items.length === 0 ? (
          <div className="flex flex-col items-start gap-1 rounded-md border border-dashed px-4 py-8">
            ...
            <p className="text-xs text-muted-foreground">Sent-back requests and pending approvals will show up here.</p>
          </div>
        ) : (
          <>
